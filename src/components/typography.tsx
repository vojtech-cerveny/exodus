...
export function H2(props: React.HTMLAttributes<HTMLHeadingElement>) {
  return (
    <h2
      {...props}
      className={cn(["scroll-m-20 pb-2 text-3xl font-semibold tracking-tight transition-colors", props.className])}
    >
      {props.children}
    </h2>
  );
...
export function Paragraph(props: React.HTMLAttributes<HTMLParagraphElement>) {
  return <p {...props} className={cn("leading-7 not-first:mt-4", props.className)} />;
}
